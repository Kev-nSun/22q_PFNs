...
#==========================================================
# SET ALLOMETRY RESULT ARROWS
#==========================================================

show_allometry_arrows <- TRUE

allometry_arrow <- setNames(
  rep("", length(PFN_names)),
  PFN_names
)

# # Hypoallometric = down arrow:
# allometry_arrow["Lang"]  <- "\u2193"
# allometry_arrow["SM-foot"] <- "\u2193"
# allometry_arrow["CingOp"] <- "\u2193"
# 
# # Hyperallometric = up arrow:
# allometry_arrow["DM-B"] <- "\u2191"
# allometry_arrow["FP-B"] <- "\u2191"
# allometry_arrow["ParM"] <- "\u2191"
# allometry_arrow["DA-ant"] <- "\u2191"

# Hypoallometric = down triangle:
allometry_arrow["Lang"]  <- "\u25BC"
allometry_arrow["SM-foot"] <- "\u25BC"
allometry_arrow["CingOp"] <- "\u25BC"

# Hyperallometric = up triangle:
allometry_arrow["DM-B"] <- "\u25B2"
allometry_arrow["FP-B"] <- "\u25B2"
allometry_arrow["ParM"] <- "\u25B2"
allometry_arrow["DA-ant"] <- "\u25B2"
...
#
# Arrows indicate network-level functional area allometry effects
#==========================================================
...
for (network in PFN_names) {
  ...
  #------------------------------------------
  # Network-level allometry arrow
  #------------------------------------------
  
  area_arrow <- ""
  
  if (show_allometry_arrows) {
    area_arrow <- allometry_arrow[network]
...
write_csv(
  ...
  file.path(
    dirout,
    "logTC_tradeoff_chord_collapsed_links_territory.csv"
  )
)
...
cairo_pdf(
  filename = file.path(
    dirout,
    "logTC_tradeoff_chord_even_networks_overlap_territory_normed_0.025_beta_triangles_sw_1000.pdf"
  ),
  width = 11,
  height = 11,
