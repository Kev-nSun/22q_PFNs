...
# This script creates data for 2 maps of PFN IDS- one corresponding to positive beta clusters of PFN loading ~ TCA, and one for negative beta clusters
...
for (PFN in c(1,2,8,9,14,15,17)) { #Only include MAIN EFFECT PFNs
...
writeMat(paste0(clust_PFN_ID_dir,"logTC_POS_PFN_ID_clust_main_effects_50mm_L.mat"), x = pos_names_list)
...
writeMat(paste0(clust_PFN_ID_dir,"logTC_POS_PFN_ID_clust_main_effects_50mm_R.mat"), x = pos_names_list)
...
writeMat(paste0(clust_PFN_ID_dir,"logTC_NEG_PFN_ID_clust_main_effects_50mm_L.mat"), x = neg_names_list)
# For R hemi, ensure that both the list and its elements are named
neg_names_list <- list()
neg_names_list$NEG_vertex_weight_clust_rh <- NEG_vertex_weight_clust_rh[,1]
writeMat(paste0(clust_PFN_ID_dir,"logTC_NEG_PFN_ID_clust_main_effects_50mm_R.mat"), x = neg_names_list)