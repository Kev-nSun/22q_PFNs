...
"""
This script compiles feature data from npz files outputted fromm "extract_PFN_loadings.py", combining features across subjects, saving it as an h5 file
"""

# Paths
BASE_DIR_PNC = Path("/cbica/projects/bbl_22q/analysis/pfn/data/PNC_PFNs_nonzero_normed_npz")
CSV_PATH  = Path("/cbica/projects/bbl_22q/analysis/allometry/inputs/filtered_demo_PNC.csv")
OUT_H5    = Path("/cbica/projects/bbl_22q/analysis/topography/inputs/PNC_filtered_compiled_vert_normed_loadings.h5")
...
def main():
    ...
    if "participant_id" not in df.columns:
        raise ValueError("CSV must have a 'participant_id' column")

    csv_ids = df["participant_id"].tolist()
    if len(csv_ids) == 0:
        raise ValueError("CSV has no rows")
    
    # Resolve to actual sub-xxxxx directory names in the same order as CSV
    sub_name = []
    ...
    with h5py.File(OUT_H5, "w") as h5:
        ...
        h5.create_dataset("participant_id", data=np.array(sub_name, dtype="S"))

        # Fill dataset
        meta_saved = False

        for index, sub in enumerate(sub_name): #loop through subjects to get npz file
