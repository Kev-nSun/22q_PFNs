...
# Rscript TCA_log_compile_PFN_loadings_gam_chunks_into_hdf5.R \
# --indir "/cbica/projects/bbl_22q/analysis/topography/results/091426_normed_TCA_vert_gam_results_nonstd_sex_avg_logTC/" \
# --out-h5 "/cbica/projects/bbl_22q/analysis/topography/results/091426_normed_TCA_vert_gam_results_nonstd_sex_avg_logTC_compiled.h5" \
# --total-len 235355 \
# --compression-level 4 \
# --chunk-rows 5000
#
...
required_first <- c("grid_n", "logTC_grid", "TC_grid_raw", "pred",
                    "age_ref", "euler_ref", "fd_ref", "isMale_ref",
                    "chunk_id", "start", "end",
                    "features_idx", "net_id", "vertex_id", "feature_col",
                    "beta_logTC", "se_logTC", "p_logTC", "edf_age", "p_age")
...
grid_n <- as.integer(first$grid_n)
...
create_pred <- function(name) {
  # dims: TOTAL_LEN x grid_n
  # chunk: CHUNK_ROWS x grid_n (whole grid per chunk row block)
  h5createDataset(
    file = OUT_H5,
    dataset = name,
    dims = c(TOTAL_LEN, grid_n),
    storage.mode = "double",    # store as double; you could use "single" if desired
    chunk = c(min(TOTAL_LEN, CHUNK_ROWS), grid_n),
    level = GZIP_LEVEL
  )
}
...
# Meta datasets
h5write(as.numeric(first$logTC_grid), OUT_H5, "meta/logTC_grid")
h5write(as.numeric(first$TC_grid_raw), OUT_H5, "meta/TC_grid_raw")
...
h5write(as.integer(grid_n), OUT_H5, "meta/grid_n")
...
create_vec("beta_logTC", "double")
create_vec("se_logTC",   "double")
create_vec("p_logTC",    "double")
...
for (k in seq_along(files)) {
  ...
  # --- write stats ---
  h5write(as.numeric(obj$beta_logTC), OUT_H5, "beta_logTC", index=list(idx))
  h5write(as.numeric(obj$se_logTC),   OUT_H5, "se_logTC",   index=list(idx))
  h5write(as.numeric(obj$p_logTC),    OUT_H5, "p_logTC",    index=list(idx))
  ...
  if (!is.matrix(mat) || nrow(mat) != p || ncol(mat) != grid_n) {
    stop(sprintf("Pred matrix mismatch in %s: got %dx%d, expected %dx%d",
                 basename(f),
                 nrow(mat), ncol(mat),
                 p, grid_n))
  }
  h5write(mat, OUT_H5, "pred/pred", index=list(idx, 1:grid_n))
