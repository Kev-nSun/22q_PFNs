...
job_keys = []
for f in surf_dir.glob("sub-*/ses-*/anat/sub-*_ses-*_hemi-L_space-fsLR_den-*_midthickness.surf.gii"):
    m = pat.match(f.name)  # match filename only
    if not m:
        continue
    job_keys.append((m["sub"], m["ses"]))
...
(deriv_dir / "22q_summary_group.json").write_text(json.dumps(results, indent=2))
...
df.to_csv(deriv_dir/"22q_summary_group.csv")