...
def name_weighted_map(sub, ses, den, atlas, net):
    ses_ent = f"_ses-{ses}" if ses else ""
    return f"sub-{sub}{ses_ent}_space-fsLR_den-{den}_atlas-{atlas}_net-{net}_weighted_area_map.dscalar.nii"
...
def process_subject(
    ...
    density: str = "32k",
    atlas: str = "PNC_group",       # <- atlas label for filenames, default is "PNC_group" but can be overwritten
):
    ...
    # Input surface paths
    surf_l = Path(surf_dir) / f"sub-{sub}/ses-{ses}/anat/sub-{sub}_ses-{ses}_hemi-L_space-fsLR_den-{density}_midthickness.surf.gii"
    surf_r = Path(surf_dir) / f"sub-{sub}/ses-{ses}/anat/sub-{sub}_ses-{ses}_hemi-R_space-fsLR_den-{density}_midthickness.surf.gii"
    ...
    for net in sorted(Path(net_dir).glob(net_glob)):
        ...
        net_label = net.name.replace("_soft_parcel_normed.dscalar.nii", "")
    ...
    return {"subject": sub, "session": ses, "TC_area": tc_area, "network_areas": weighted_sums}
