...
def name_vertex_area_metric(sub, ses, den, hemi):
    ...
    return (
        f"sub-{sub}{ses_ent}_space-fsLR_den-{den}_hemi-{hemi}"
        "_desc-template_vertex_area_metric.shape.gii"
    )
...
def name_vertex_area_dscalar(sub, ses, den):
    ...
    return (
        ...
        "_desc-template_vertex_area_map.dscalar.nii"
    )
...
def name_weighted_map(sub, ses, den, atlas, net):
    ...
    return (
        ...
        f"_net-{net}_desc-template_weighted_area_map.dscalar.nii"
    )
...
def surface_vertex_areas(
    ...
    """
    Compute geometric vertex-wise surface area directly from a surface mesh.

    Workbench assigns each vertex one third of the area of each triangle
    incident on that vertex.

    Output units are mm^2.
    """
...
def process_subject(
    ...
    """
    Compute fsLR template-surface-area-weighted network areas.

    This version uses the actual geometric vertex areas of the
    TemplateFlow fsLR midthickness surface.

    Workflow:
        1. Fetch TemplateFlow fsLR midthickness .surf.gii files.
        2. Compute vertex areas using:
               wb_command -surface-vertex-areas
        3. Combine L/R vertex-area metrics into a cortical CIFTI.
        4. Multiply template geometric vertex area by each network loading.
        5. Sum across cortex.

    This does NOT use TemplateFlow desc-vaavg.

    Therefore these values reflect the literal geometric area of the
    distributed fsLR template midthickness mesh, not the group-average
    HCP subject vertex-area metric.
    """
    ...
    # ------------------------------------------------------------
    # Fetch actual TemplateFlow fsLR midthickness surfaces
    # ------------------------------------------------------------
    ...
    # ------------------------------------------------------------
    # Link/copy template surfaces into derivative tree
    # ------------------------------------------------------------
    ...
    # ------------------------------------------------------------
    # Compute geometric vertex areas from template surface
    # ------------------------------------------------------------
    ...
    # ------------------------------------------------------------
    # Combine L/R vertex areas into dense scalar
    # ------------------------------------------------------------
    ...
    # ------------------------------------------------------------
    # Total cortical area of the template geometry
    # ------------------------------------------------------------
    ...
    for net in sorted(
        ...
        net_label = net.name.replace(
            "_soft_parcel_normed.dscalar.nii",
            "",
        )
