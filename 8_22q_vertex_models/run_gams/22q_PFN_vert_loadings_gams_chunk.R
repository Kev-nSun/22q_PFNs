...
# This scripts also gives model-fit predictions based on 22q status
...
required_cols <- c("subject", "stat_22q", "sex", "euler", "median_fd_combined", "age", "TC_area", "subclass")
missing_cols <- setdiff(required_cols, names(covar))
if (length(missing_cols) > 0) {
  stop(paste0("22q_PNC_matched_all.csv missing required columns: ", paste(missing_cols, collapse=", ")))
}
...
# ---------------------------
#h5_ids_raw <- h5read(opt$h5, "subject")
...
logTC_ref <- median(all_phenos$logTC, na.rm=TRUE)
...
make_newdata <- function() {
  ...
  data.frame(
    ...
    logTC = rep(logTC_ref, length(status)),
    ...
    subclass = factor(rep(subclass_ref, length(status)), levels = levels(all_phenos$subclass))
  )
}
...
cov_template <- as.data.frame(
  all_phenos[, .(stat_22q, logTC, isMale, euler, median_fd_combined, age, subclass)]
)
...
fit_one <- function(y) {
  ...
  mod <- bam(
    Y ~ stat_22q + logTC + isMale + euler + median_fd_combined + s(age, k=4) + s(subclass, bs = "re"),
    data = dat2,
    method = "fREML",
    discrete = TRUE
  )
...
for (feat in seq_len(feat_n)) {
  ...
  # Predictions
  nd <- make_newdata()
  # note: nd uses column names matching formula
  pred_mat[feat, ] <- predict(
    mod,
    newdata = nd,
    type = "response",
    exclude = "s(subclass)" #marginalize away subclass effects
  )
