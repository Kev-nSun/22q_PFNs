---
title: "demographics_22q_median-FD"
...
### FILTERED 22q LIST WITH MEDIAN FD ###
# filtered by having at least idemo or rest scan, each at least 3 min scan time, and having associated field map

# Source: /cbica/projects/bbl_22q/data/22q_xcpd_extracted
current_22q_list <- read.table('C:/Users/kevin/OneDrive/Documents/NGG_PhD/Alexander-Bloch/22q_Project/QC/22q_median_fd_summary.csv', sep=',', header=TRUE) #114
```

## Perform basic preprocessing
```{r}
# Remove sub-016095 ses-008971 (2 sessions, remove this one it is missing resting state)
...
```{r}
### Add in median FD
median_FD <- current_22q_list[, c("sub", "median_fd_combined", "median_fd_idemo", "median_fd_rest", "n_vols_idemo", "n_vols_rest")]
demo_22q <- merge(demo_22q, median_FD, by = "sub", all.x = TRUE)
...
### Save
write.csv(demo_22q,'C:/Users/kevin/OneDrive/Documents/NGG_PhD/Alexander-Bloch/22q_Project/QC/demo_22q.csv', row.names = FALSE)
```
...
Fig
ggsave(paste0("../Histograms/22q_age.png"), width = 19, height = 16, dpi = 600, units = "cm")


age_range_matched_22q <- demo_22q[demo_22q$age < 23, ] # N = 65

#Get age histogram under 23 years old
Fig <- ggplot(age_range_matched_22q, aes(x = age)) +
  geom_histogram(binwidth = 1,    # Width of the bins
                 fill = "gray", # Fill color of the bars
                 color = "black") + # Border color of the bars
  labs(title = "22q Distribution of Age, Age < 23",
       x = "Age",
       y = "Number of Subjects") +
  theme(plot.title = element_text(hjust = 0.5)) +
...
Fig
ggsave(paste0("../Histograms/22q_age_under_23.png"), width = 19, height = 16, dpi = 600, units = "cm")
mean_age_under_23 <- mean(age_range_matched_22q$age)

#Get motion stats for those under 23
median_fd_comb_avg <- mean(age_range_matched_22q$median_fd_combined)
median_fd_comb_SD <- sd(age_range_matched_22q$median_fd_combined)

median_fd_idemo_avg <- mean(age_range_matched_22q$median_fd_idemo, na.rm = TRUE)
median_fd_idemo_SD <- sd(age_range_matched_22q$median_fd_idemo, na.rm = TRUE)

median_fd_rest_avg <- mean(age_range_matched_22q$median_fd_rest, na.rm = TRUE)
median_fd_rest_SD <- sd(age_range_matched_22q$median_fd_rest, na.rm = TRUE)

avg_22q <- c(median_fd_comb_avg, median_fd_idemo_avg, median_fd_rest_avg)
SD_22q <- c(median_fd_comb_SD, median_fd_idemo_SD, median_fd_rest_SD)

median_fd_df <- data.frame(avg_22q,SD_22q)
rownames(median_fd_df) <- c("combined","idemo","rest")

write.csv(median_fd_df,"../median_FD_stats_22q_under_23.csv")

comb_22q <- ggplot(age_range_matched_22q, aes(x = median_fd_combined)) +
  geom_histogram(binwidth = 0.01,    # Width of the bins
                 fill = "gray", # Fill color of the bars
                 color = "black") + # Border color of the bars
  labs(title = "22q Distribution of Median FD Across Concatenated Scans, Age < 23",
       x = "Median FD",
       y = "Number of Subjects") +
  theme(plot.title = element_text(hjust = 0.5)) +
  scale_y_continuous(breaks = c(0, 2, 4, 6, 8, 10)) # Specify desired tick mark positions
comb_22q
ggsave(paste0("../Histograms/median_FD/22q_median_FD_combined_10-23yo.png"), width = 19, height = 16, dpi = 600, units = "cm")

idemo_22q <- ggplot(age_range_matched_22q, aes(x = median_fd_idemo)) +
  geom_histogram(binwidth = 0.01,    # Width of the bins
                 fill = "gray", # Fill color of the bars
                 color = "black") + # Border color of the bars
  labs(title = "22q Distribution of Median FD in IDEMO scan, Age < 23",
       x = "Median FD",
       y = "Number of Subjects") +
  theme(plot.title = element_text(hjust = 0.5)) +
  scale_y_continuous(breaks = c(0, 2, 4, 6, 8, 10, 12)) # Specify desired tick mark positions
idemo_22q
ggsave(paste0("../Histograms/median_FD/22q_median_FD_idemo_10-23yo.png"), width = 19, height = 16, dpi = 600, units = "cm")

rest_22q <- ggplot(age_range_matched_22q, aes(x = median_fd_rest)) +
  geom_histogram(binwidth = 0.01,    # Width of the bins
                 fill = "gray", # Fill color of the bars
                 color = "black") + # Border color of the bars
  labs(title = "22q Distribution of Median FD in Rest scan, Age < 23",
       x = "Median FD",
       y = "Number of Subjects") +
  theme(plot.title = element_text(hjust = 0.5)) +
  scale_y_continuous(breaks = c(0, 2, 4, 6, 8, 10)) # Specify desired tick mark positions
rest_22q
ggsave(paste0("../Histograms/median_FD/22q_median_FD_rest_10-23yo.png"), width = 19, height = 16, dpi = 600, units = "cm")
